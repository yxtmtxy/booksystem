...
<script>
    layui.use(['form', 'table'], function () {
        ...
        table.render({
            elem: '#currentTableId',
            url: '${pageContext.request.contextPath}/applyTeaAll',//查询全部数据
            toolbar: '#toolbarDemo',
            ...
            if (obj.event === 'edit') {  // 监听修改操作
                if(data.type==1){
                    ...
                    var index = layer.open({
                        ...
                        shadeClose: true,
                        area: ['100%', '100%'],
                        content: '${pageContext.request.contextPath}/queryApplyTeaById?id='+data.id,
                    });
        ...
        table.on('toolbar(currentTableFilter)', function (obj) {
            if (obj.event === 'add') {  // 监听添加操作
                var index = layer.open({
                    ...
                    shadeClose: true,
                    area: ['60%', '60%'],
                    content: '${pageContext.request.contextPath}/applyTeaAdd',
                });
                $(window).on("resize", function () {
                    layer.full(index);
                });
