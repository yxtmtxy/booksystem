        ...
        var options = {
            <c:choose>
                ...
                <c:when test="${sessionScope.type.equals('admin')}">
                    iniUrl: "${pageContext.request.contextPath}/api/init0.json",    // 初始化接口，系统管理员
                </c:when>
                <c:when test="${sessionScope.type.equals('officer')}">
                    iniUrl: "${pageContext.request.contextPath}/api/init1.json",    // 初始化接口，教务处
                </c:when>
                <c:when test="${sessionScope.type.equals('teacher')}">
                    iniUrl: "${pageContext.request.contextPath}/api/init2.json",    // 初始化接口，教师
                </c:when>
                <c:when test="${sessionScope.type.equals('student')}">
                    iniUrl: "${pageContext.request.contextPath}/api/init3.json",    // 初始化接口，学生
                 </c:when>
